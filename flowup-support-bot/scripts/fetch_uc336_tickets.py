#!/usr/bin/env python3
"""
Script pour récupérer 10 tickets UC 336 depuis Odoo
Filtres: UC 336, créés entre janvier 2024 et septembre 2025
"""
...
class OdooTicketFetcher:
    """Récupérateur de tickets Odoo avec filtres spécifiques"""
    ...
    def fetch_uc336_tickets(self, limit: int = 10) -> List[Dict]:
        """
        Récupère les tickets UC 336 avec filtres de date
        ...
        try:
            ...
            # URL pour récupérer les tickets
            ...
            if response.status_code == 200:
                ...
                if 'result' in result:
                    ...
                    print(f"❌ Aucun ticket trouvé: {result}")
                    return []
            else:
        ...
        try:
            ...
            for message in messages:
                if message.get('message_type') == 'comment':
                    ...
                    if body and not body.startswith('<p>') and len(body.strip()) > 10:
                        # Nettoyer le HTML basique
                        clean_body = body.replace('<p>', '').replace('</p>', '').strip()
                        if clean_body:
                            return clean_body
...
def main():
    """Fonction principale"""
    print("🎫 RÉCUPÉRATION TICKETS UC 336")
    print("=" * 50)
    
    # Initialiser le récupérateur
    fetcher = OdooTicketFetcher()
    ...
    output_file = "/Users/r4v3n/Workspce/Prod/kikoo_rag/flowup-support-bot/data/uc336_tickets_fetched.json"
